  ...
  say $q->p(
    $q->table(
      ...
      $q->Tr(
        ...
        $q->td(
          $q->password_field( -name => 'password', -size => 30 )
        )
      )
    )
  );
...
sub print_page_header {
  ...
  # CGI is a bit old and doesn't support the crossorigin or async attributes here.  Sigh.
  my $head_js = <<HEADJS;
<script async src="https://pagead2.googlesyndication.com/pagead/js/adsbygoogle.js?client=ca-pub-6096802328354859" crossorigin="anonymous"></script>
HEADJS
  $head =~ s!(</script>)!>$1\n$head_js!;
  say $head;
  
  say '<div id="fb-root"></div>';
  say '<script async defer crossorigin="anonymous" src="https://connect.facebook.net/en_GB/sdk.js#xfbml=1&version=v11.0" nonce="w0AtEfUS"></script>';
  
  say $q->h1( 'Wordfeud Tile Tracker' );
  
  # Buy me a coffee!
  say $q->p( 'This site will always be free but if you are a fan and would like to contribute to the running costs then please simply click on an advert or feel free to:' );
  say $q->p( $q->a( { href => 'https://ko-fi.com/A30243J1', target => '_blank' }, '<img height="36" style="border:0px;height:36px;" src="https://storage.ko-fi.com/cdn/kofi4.png" border="0" alt="Buy Me a Coffee" />' ) );
  say $q->p( 'All donations are very gratefully received.' );

  say <<INLINEAD;
<script async src="https://pagead2.googlesyndication.com/pagead/js/adsbygoogle.js?client=ca-pub-6096802328354859"
     crossorigin="anonymous"></script>
<ins class="adsbygoogle"
     style="display:block"
     data-ad-format="fluid"
     data-ad-layout-key="-gm-3+1f-3d+2z"
     data-ad-client="ca-pub-6096802328354859"
     data-ad-slot="6277955621"></ins>
<script>
...
sub print_page_footer {
  ...
  say $q->p( 'Please follow/like and leave us any feedback/comments/suggestions on our',
...
INLINEAD
  ...
  say $q->p( $q->small( 'Timestamps are all in GMT.' ) );
  say $q->p( $q->small( 'Current time is ' . gmtime->strftime( "%F %T" ) . '.' ) );
  say $q->p( $q->small( 'Page generated in ' . tv_interval( $wf->{t0} ) . ' seconds.' ) );
  say $q->p( $q->small( 'This site is provided by ardavey free of charge with no guarantees of making you a better player.' ) );
